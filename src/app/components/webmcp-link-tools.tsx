...
function conciseLink(link: LinkDTO, linkHost: string): string {
  const address = link.domain ? `${link.domain}/${link.slug}` : `${linkHost}/${link.slug}`;
...
export function WebMcpLinkTools() {
      ...
      },
      {
        ...
        execute: (input, options) =>
          withLink(deleteLinkInput, input, options?.signal, async (link) => {
            ...
            await navigate({ to: "/links" });
            return toolResult(
              `Deleted ${link.domain ?? linkHost}/${link.slug}. It no longer redirects.`,
            );
          }),
      },
    ];
  ...
  }, [currentUser.data, navigate, org, queryClient, linkHost]);
